...
// Returns the color matching the current battery level using a 4-step gradient.
// Consistent between the bar fill and the percentage label.
static lv_color_t battery_level_color(uint8_t level)
{
    if (level <= 10) return lv_palette_main(LV_PALETTE_RED);
    if (level <= 25) return lv_color_hex(0xFF8000); // Orange
    if (level <= 50) return lv_palette_main(LV_PALETTE_YELLOW);
...
static void set_battery_symbol(lv_obj_t *widget, struct battery_state state)
{
    ...
    if (state.level < 1)
    {
        // Peripheral disconnected — hide both elements
    ...
    // Label uses the same 4-step color as the bar for visual consistency
...
int zmk_widget_dongle_battery_status_init(struct zmk_widget_dongle_battery_status *widget, lv_obj_t *parent)
{
    ...
    int total = ZMK_SPLIT_CENTRAL_PERIPHERAL_COUNT + SOURCE_OFFSET;
    int cols = (total > 2) ? 2 : total;
    int rows = (total + cols - 1) / cols;
    // row_h must fit: 5px bar + gap + ~20px label text
    int row_h = 28;

    lv_obj_set_size(widget->obj, 240, rows * row_h);
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        int col = i % cols;
        int row = i / cols;
        int x_off = (cols == 1) ? 0 : (-60 + col * 120);

        // Bar on top, label text below
        lv_obj_align(image_canvas, LV_ALIGN_TOP_MID, x_off, row * row_h + 2);
        lv_obj_align(battery_label, LV_ALIGN_TOP_MID, x_off, row * row_h + 8);
