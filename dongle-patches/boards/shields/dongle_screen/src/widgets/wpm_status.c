...
// Color ramp based on typing speed:
// idle → gray, slow → white, good → yellow, fast → orange, blazing → green
static lv_color_t wpm_color(int wpm)
{
    if (wpm <= 0)  return lv_color_hex(0x606060); // Gray   — idle
    if (wpm <= 30) return lv_color_white();         // White  — learning
    if (wpm <= 50) return lv_palette_main(LV_PALETTE_YELLOW); // Yellow — steady
    if (wpm <= 70) return lv_color_hex(0xFF8000);  // Orange — fast
    return lv_palette_main(LV_PALETTE_GREEN);        // Green  — blazing
}
...
static void set_wpm(struct zmk_widget_wpm_status *widget, struct wpm_status_state state)
{
    ...
    lv_obj_set_style_text_color(widget->wpm_label, wpm_color(state.wpm), 0);
}
